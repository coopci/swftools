...
const char * lookup_method_name( abc_file_t * srcabc , abc_method_t * m ){
    
    int i = 0;
    
    for ( i =0 ; i < srcabc->classes->num ; ++i){
        abc_class_t * the_class = (abc_class_t*)array_getvalue(srcabc->classes, i);
        
        if ( the_class->constructor == m) {
            return the_class->classname->name;
        }
        
        if ( the_class->static_constructor == m) {
            return the_class->classname->name;
        }
        
        trait_list_t * traits = the_class->traits;
        while(traits) {
            if ( traits->trait->kind == 1 && traits->trait->method == m ){
                return traits->trait->name->name ;
            }
            traits = traits->next;
        }
        
        traits = the_class->static_traits;
        while(traits) {
            if ( traits->trait->kind == 1 && traits->trait->method == m ){
                return traits->trait->name->name ;
            }
...
abc_file_t * extractClass(abc_file_t * srcabc){
    int i =0;
    abc_file_t * dstabc = abc_file_new();
    
    dstabc->flags = srcabc->flags;
    abc_class_t* the_class = 0;
    for ( i =0 ; i < srcabc->classes->num ; ++i){
        abc_class_t * c = (abc_class_t*)array_getvalue(srcabc->classes, i);
        //array_append( dstabc->classes, NO_KEY, c);
       
        if ( !strcmp (c->classname->name , extractname ) ){
            ...
            printf("traits: \n");
            while(traits) {
                printf("\t trait: %d %s  \n" , traits->trait->kind,traits->trait->name->name );
                traits = traits->next;
            }
            
            traits = c->static_traits;
            printf("static_traits: \n");
            while(traits) {
                printf("\t trait: %d\n" , traits->trait->kind );
                traits = traits->next;
            }
    ...
    abc_method_t * entry_point = 0;   
    for ( i =0 ; i < srcabc->methods->num ; ++i){
    
        if ( !the_class )
            break;
        abc_method_t * c = (abc_class_t*)array_getvalue(srcabc->methods, i);
        
        if ( the_class->constructor == c) {
            array_append( dstabc->methods, NO_KEY, c);
            array_append( dstabc->method_bodies, NO_KEY, c->body);
            continue;
        }
        
        if ( the_class->static_constructor == c) {
            entry_point = c;
            array_append( dstabc->methods, NO_KEY, c);
            array_append( dstabc->method_bodies, NO_KEY, c->body);
            continue;
        ...
        trait_list_t * traits = the_class->traits;
        while(traits) {
            if ( traits->trait->kind == 1 && traits->trait->method == c ){
                array_append( dstabc->methods, NO_KEY, c);
                array_append( dstabc->method_bodies, NO_KEY, c->body);
            }
            traits = traits->next;
    ...
    for ( i =0 ; i < srcabc->scripts->num ; ++i){
        if ( 0 && i >= srcabc->scripts->num - 1 ) {// entry point
            abc_script_t * s = (abc_class_t*)array_getvalue(srcabc->scripts, i);
            {
                printf("entry point \n");
                
                dump_method( stdout, "dump method", 
                                 "", 
                                 "", 
                                 "", 
                                 s->method, srcabc, 0);
                
                trait_list_t * traits = s->traits;
                while(traits) {
                    printf("\t trait %d\n" ,  traits->trait->kind);
                    if ( traits->trait->kind == 4){
                        printf("\t\t class %s\n" ,  traits->trait->cls->classname->name);
                        traits->trait->cls = the_class;
                        traits->trait->method = entry_point;
                    }
                    traits = traits->next;
                }
                
                array_append( dstabc->scripts, NO_KEY, s);
            }
        }
        if ( 1) {// find the script for the_class
            abc_script_t * s = (abc_class_t*)array_getvalue(srcabc->scripts, i);
            {
                trait_list_t * traits = s->traits;
                while(traits) {
                    if (traits->trait->cls == the_class){
                        printf("found the script for the_class\n");
                        array_append( dstabc->scripts, NO_KEY, s);
                        array_append( dstabc->methods, NO_KEY, s->method);
                        array_append( dstabc->method_bodies, NO_KEY, s->method->body);
                        break;
                    }
                    traits = traits->next;
                }
    ...
    /*
    for ( i =0 ; i < srcabc->method_bodies->num ; ++i){
        if ( !the_class )
            break;
        abc_method_body_t * c = (abc_class_t*)array_getvalue(srcabc->method_bodies, i);
        {
            array_append( dstabc->method_bodies, NO_KEY, c);
        }
    }     */   
...
void extractTagDoABC(SWF*swf, char*filename , TAG* doabctag){
    SWF newswf;
    TAG*desttag;
    TAG*srctag;
    RGBA rgb;
    ...
    while(srctag) {
        if ( srctag->id == ST_DOABC){
            abc_file_t * abc = swf_ReadABC(srctag);
            ...
            */
            ...
            for ( i =0 ; i < abc->classes->num ; ++i){
                abc_class_t * c = (abc_class_t*)array_getvalue(abc->classes, i);
            ...
            for ( i =0 ; i < abc->methods->num ; ++i){
                abc_method_t * m = (abc_method_t*)array_getvalue(abc->methods, i);
                
                printf("%d , %s \n" , i , 
                lookup_method_name( abc ,  m )
                );
            }
            ...
            abc_file_t * dstabc = extractClass( abc );
            ...
            swf_WriteABC( desttag , dstabc );
            
            /*
            desttag->len = desttag->memsize = srctag->len;
            desttag->data = malloc(srctag->len);
            
            swf_SetTagPos(desttag, 0);
            swf_SetU32(desttag , 0); 
        ...
        if ( //srctag->id == ST_SYMBOLCLASS ||
            //srctag->id == ST_EXPORTASSETS || 
            srctag->id == ST_SCRIPTLIMITS ||
            srctag->id == ST_FILEATTRIBUTES 
            ){
            TAG*ttag = (TAG*)malloc(sizeof(TAG));
            desttag = swf_InsertTag(desttag, srctag->id);
            desttag->len = desttag->memsize = srctag->len;
...
{ 
    ...
    */
    ...
    if (founddoabc)
	extractTagDoABC(&swf, destfilename , doabctag);
